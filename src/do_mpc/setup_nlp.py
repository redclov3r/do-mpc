...
def setup_nlp(model, optimizer):
    ...
    for jj in range(len(p_scenario_index)):
        for ii in range(np):
            p_scenario[jj, ii] = uncertainty_values[
                ii][p_scenario_index[jj, ii]]
            ...
            for b in range(n_branches[k]):
                ...
                if soft_constraint:
                    ...
                    for index_soft in range(cons.size1()):
                        J_ksb_soft = penalty_term_cons[
                            index_soft] * (EPSILON[index_soft])**2
                        J += J_ksb_soft
