...
class mpc_data:
    ...
    def __init__(self, configuration):
        ...
        self.mpc_states[0, :] = configuration.model.ocp.x0 / \
            configuration.model.ocp.x_scaling
        self.mpc_control[0, :] = configuration.model.ocp.u0 / \
            configuration.model.ocp.u_scaling
        self.mpc_time[0] = 0
...
def plot_mpc(configuration):
    ...
    for index in range(len(plot_states)):
        ...
        plt.plot(mpc_time[0:index_mpc], mpc_states[
                 0:index_mpc, plot_states[index]] * x_scaling[plot_states[index]])
    ...
    for index in range(len(plot_control)):
        ...
        plt.plot(mpc_time[0:index_mpc], mpc_control[0:index_mpc, plot_control[
                 index]] * u_scaling[plot_control[index]], drawstyle='steps')
...
def plot_control_pred(v, t0, el, lineop, n_scenarios, n_branches, nk, parent_scenario, U_offset, u_scaling, t_step, u_last_step):
    ...
    for k in range(nk):
        ...
        for s in range(n_scenarios[k]):
            ...
            plt.plot(NP.array([t_beginning, t_end]),
                     NP.array([u_this, u_this]), lineop)

            # Plot vertical line connecting the scenarios
            if k == 0:
                u_prev = u_last_step
            else:
                u_prev = v[el + U_offset[k - 1]
                           [parent_scenario[k][s]]] * u_scaling[el]
            plt.plot(NP.array([t_beginning, t_beginning]),
                     NP.array([u_prev, u_this]), lineop)
...
def plot_animation(configuration):
    ...
    if configuration.simulator.plot_anim:
        ...
        parent_scenario = configuration.optimizer.nlp_dict_out[
            'parent_scenario']
        ...
        for index in range(len(plot_states)):
            ...
            plot_state_pred(v_opt, t0, plot_states[
                            index], '-b', n_scenarios, n_branches, nk, child_scenario, X_offset, x_scaling, t_step)
            plt.plot(mpc_time[0:index_mpc], mpc_states[0:index_mpc, plot_states[
                     index]] * x_scaling[plot_states[index]], '-k', linewidth=2.0)
            plt.ylabel(str(x[plot_states[index]]))
            plt.xlabel("Time")
            plt.grid()
            plot.yaxis.set_major_locator(MaxNLocator(4))
        ...
        for index in range(len(plot_control)):
            plot = plt.subplot(total_subplots, 1, len(plot_states) + index + 1)
            # First plot the prediction
            plot_control_pred(v_opt, t0, plot_control[index], '-b', n_scenarios, n_branches, nk,
                              parent_scenario, U_offset, u_scaling, t_step, mpc_control[index_mpc - 1, plot_control[index]])
            plt.plot(mpc_time[0:index_mpc], mpc_control[0:index_mpc, plot_control[
                     index]] * u_scaling[plot_control[index]], '-k', drawstyle='steps', linewidth=2.0)
            plt.ylabel(str(u[plot_control[index]]))
            plt.xlabel("Time")
            plt.grid()
            plot.yaxis.set_major_locator(MaxNLocator(4))
